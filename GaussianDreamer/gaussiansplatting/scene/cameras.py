...
class My_Camera(nn.Module):
    def __init__(self, c2w, fov, height, width, fov_type='fovx',
                 trans=torch.tensor([0.0, 0.0, 0.0]), scale=1.0, data_device="cuda"):
        super(Camera, self).__init__()
        ...
        if fov_type == 'fovx':
            ...
            raise ValueError("fov_type must be either 'fovx' or 'fovy'")
        ...
        self.world_view_transform = getWorld2View2_tensor(R, T).transpose(0, 1).float().to(self.data_device)
        self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0, 1).float().to(self.data_device)
        self.full_proj_transform = (self.world_view_transform.unsqueeze(0).bmm(self.projection_matrix.unsqueeze(0))).squeeze(0).float()
        self.camera_center = self.world_view_transform.inverse()[3, :3].float()
